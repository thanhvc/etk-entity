...
public class TimeDuration implements Serializable, Comparable<TimeDuration> {
  ...
  /**
   * Elapsed time constructor. The time duration will be computed from the two
  ...
  public TimeDuration(Calendar cal1, Calendar cal2) {
    ...
    /*
     * Strategy: Using millisecond arithmetic alone will produce inaccurate
     * results. Using a Calendar alone will take too long. So, we use
     * millisecond arithmetic to get near the correct result, then zero in on
     * the correct result using a Calendar.
     */
  ...
  public boolean equals(Object obj) {
    ...
    try {
      ...
      return this.years == that.years && this.months == that.months && this.days == that.days
          ...
          && this.seconds == that.seconds && this.milliseconds == that.milliseconds;
    } catch (Exception e) {
    }
    return false;
  }
  ...
  public boolean isZero() {
    return this.milliseconds == 0 && this.seconds == 0 && this.minutes == 0 && this.hours == 0
        && this.days == 0 && this.months == 0 && this.years == 0;
  }
   ...
   * <code>long</code> value. This method is intended to be used in tandem with
   * the <code>toLong</code> method. <b>Note:</b> this method should not be used
   * to calculate elapsed time - use the elapsed time constructor instead.
   * 
   ...
   * <code>Number</code> instance. If <code>number</code> is <code>null</code>,
   * returns a zero <code>TimeDuration</code>.
   * <p>
   * This is a convenience method intended to be used with entity engine fields.
   * Some duration fields are stored as a <code>Long</code>, while others are
   * stored as a <code>Double</code>. This method will decode both types.
   * </p>
   * 
  ...
  public static TimeDuration parseDuration(String duration) {
    ...
    return new TimeDuration(intArray[0],
                            intArray[1],
                            intArray[2],
                            intArray[3],
                            intArray[4],
  ...
  public static long toLong(TimeDuration duration) {
    return (0x757B12C00L * duration.years) + (0x9CA41900L * duration.months)
        + (86400000 * (long) duration.days) + (3600000 * (long) duration.hours)
        + (60000 * (long) duration.minutes) + (1000 * (long) duration.seconds)
