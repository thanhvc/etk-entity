...
public class Deserializer {
  /**
   * The name of the variable to use to get the current container name as a
   * suffix if the current container is a portal container, the value of the
   * variable will be "-${portal-container-name}", it will be an empty String
   * otherwise
   */
  public static final String EXO_CONTAINER_PROP_NAME  = "container.name.suffix";

  /**
   * The prefix of the name of all the variables tied to the current portal
   * container
   */
  public static final String PORTAL_CONTAINER_VARIABLE_PREFIX = "portal.container.";

  /**
   * Resolve a string value. If the input value is null then the returned value
   * is null.
   * 
   ...
   * <li>If the value is equals to the string litteral true ignoring the case
   * then true is returned</li>
   * <li>If the value is equals to the string litteral false ignoring the case
   ...
   * <ol>
   * <li>Resolve any system property in the input value</li>
   * <li>Attempt to parse the value using the {@link Integer#parseInt(String)}
   * method and returns its value.</li>
   * <li>If the parsing fails then throws an
   ...
   * <ol>
   * <li>Resolve any system property in the input value</li>
   * <li>Attempt to parse the value using the {@link Long#parseLong(String)}
   * method and returns its value.</li>
   * <li>If the parsing fails then throws an
   * <code>IllegalArgumentException</code></li>
   * </ol>
   ...
   * <ol>
   * <li>Resolve any system property in the input value</li>
   * <li>Attempt to parse the value using the {@link Double#parseDouble(String)}
   * method and returns its value.</li>
   * <li>If the parsing fails then throws an
   * <code>IllegalArgumentException</code></li>
   * </ol>
   ...
   * composed of the system properties and the portal container settings
   * 
        ...
        if (start + 2 == i) {
          ...
          if (key.equals(Deserializer.EXO_CONTAINER_PROP_NAME)) {
            ...
            if (container instanceof ApplicationContainer) {
              ...
              RootContainer rootContainer = (RootContainer) KernelContainerContext.getTopContainer();
            ...
            // We try to get a value tied to the current portal
            // container.
            KernelContainer container = KernelContainerContext.getCurrentContainerIfPresent();
            if (container instanceof ApplicationContainer) {
              // The current container is a portal container
              Object oValue = ((ApplicationContainer) container).getSetting(key.substring(Deserializer.PORTAL_CONTAINER_VARIABLE_PREFIX.length()));
              value = oValue == null ? null : oValue.toString();
            }
          } else {
            if (props != null) {
              // Some parameters have been given thus we need to
              // check inside first
              Object oValue = props.get(key);
              value = oValue == null ? null : oValue.toString();
            }
            if (value == null) {
              // No value could be found so far, thus we try to
              // get it from the
              // system properties
  ...
  /**
   * This methods will remove useless characters from the given {@link String}
   * and return the result
   * 
   * @param s the input value
   * @return <code>null</code> if the input value is <code>null</code>,
   *         <code>s.trim()</code> otherwise
   */
  ...
  /**
   * This method will first resolves the variables then it will clean the
   * results
   * 
   * @param s the input value
