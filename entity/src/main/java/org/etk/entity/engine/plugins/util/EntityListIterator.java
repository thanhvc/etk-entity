...
public class EntityListIterator implements ListIterator<GenericValue> {
  ...
  public EntityListIterator(ResultSet resultSet,
                            Entity modelEntity,
                            List<Field> selectFields,
                            ModelFieldTypeReader modelFieldTypeReader) {
    this.sqlp = null;
  ...
  public void afterLast() throws GenericEntityException {
    try {
      ...
      if (!closed) {
        this.close();
        Debug.logWarning("Warning: auto-closed EntityListIterator because of exception: "
                             + e.toString(),
                         module);
      }
  ...
  /**
   * Sets the cursor position to just before the first result so that next()
  ...
  /**
   * Sets the cursor position to last result; if result set is empty returns
   * false
   */
  ...
  /**
   * Sets the cursor position to first result; if result set is empty returns
   * false
   */
  ...
  public void close() throws GenericEntityException {
    if (closed) {
      // maybe not the best way: throw new
      // GenericResultSetClosedException("This EntityListIterator has been closed, this operation cannot be performed");
      Debug.logWarning("This EntityListIterator for Entity [" + modelEntity == null ? ""
                                                                                   : modelEntity.getEntityName()
                                                                                       + "] has already been closed, not closing again.",
                       module);
    } else {
      if (sqlp != null) {
        sqlp.close();
  ...
  /**
   * NOTE: Calling this method does return the current value, but so does
   * calling next() or previous(), so calling one of those AND this method will
   * cause the value to be created twice
   */
  public GenericValue currentGenericValue() throws GenericEntityException {
    ...
    for (int j = 0; j < selectFields.size(); j++) {
      Field curField = selectFields.get(j);
   ...
   * rowNum is negative, goes to that position relative to the end of the list;
   ...
   * positive, goes forward relative to the current position; if rows is
   * negative, goes backward relative to the current position;
   */
   ...
   * method can be very inefficient; it is much better to just use next() until
   * it returns null For example, you could use the following to iterate through
   * the results in an EntityListIterator: GenericValue nextValue = null; while
   * ((nextValue = (GenericValue) this.next()) != null) { ... }
   */
  public boolean hasNext() {
    if (!haveShowHasNextWarning) {
      // DEJ20050207 To further discourage use of this, and to find existing
      // use, always log a big warning showing where it is used:
      Exception whereAreWe = new Exception();
      Debug.logWarning(whereAreWe,
                       "WARNING: For performance reasons do not use the EntityListIterator.hasNext() method, just call next() until it returns null; see JavaDoc comments in the EntityListIterator class for details and an example",
                       module);
    ...
    try {
      if (resultSet.isLast() || resultSet.isAfterLast()) {
        ...
        // if we are not in the first or beforeFirst positions and we haven't
        // made any values yet, the result set is empty so return false
      ...
      if (!closed) {
        try {
          ...
          Debug.logError(e1,
                         "Error auto-closing EntityListIterator on error, so info below for more info on original error; close error: "
                             + e1.toString(),
                         module);
        }
        Debug.logWarning("Warning: auto-closed EntityListIterator because of exception: "
  ...
  public boolean hasPrevious() {
    try {
      if (resultSet.isFirst() || resultSet.isBeforeFirst()) {
        ...
        // do a quick game to see if the resultSet is empty:
        // if we are not in the last or afterLast positions and we haven't made
        // any values yet, the result set is empty so return false
      ...
      throw new GeneralRuntimeException("Error while checking to see if this is the first result",
                                        e);
    }
  }

  /**
   * Moves the cursor to the next position and returns the GenericValue object
   * for that position; if there is no next, returns null For example, you could
   * use the following to iterate through the results in an EntityListIterator:
   * GenericValue nextValue = null; while ((nextValue = (GenericValue)
   * this.next()) != null) { ... }
   */
  ...
  /**
   * Returns the index of the next result, but does not guarantee that there
   * will be a next result
   */
  ...
  /**
   * Moves the cursor to the previous position and returns the GenericValue
   * object for that position; if there is no previous, returns null
   */
  ...
  public List<GenericValue> getCompleteList() throws GenericEntityException {
    try {
      // if the resultSet has been moved forward at all, move back to the
      // beginning
  ...
  public List<GenericValue> getPartialList(int start, int number) throws GenericEntityException {
    try {
      ...
      // if starting on result 1 just call next() to avoid scrollable issues in
      // some databases
      if (start == 1) {
        if (!resultSet.next()) {
          return list;
        ...
        if (!this.absolute(start)) {
          ...
          return list;
          // throw new
          // GenericEntityException("Could not move to the start position of " +
          // start +
      ...
      // number > numRetreived comparison goes first to avoid the unwanted call
      // to next
  ...
  public int getResultsSizeAfterPartialList() throws GenericEntityException {
    if (genericDAO != null) {
      if (resultSize == null) {
        ...
        resultSize = (int) genericDAO.selectCountByCondition(modelEntity,
                                                             whereCondition,
                                                             havingCondition,
                                                             selectFields,
                                                             efo);
